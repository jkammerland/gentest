...
string(FIND "${_meson_content}" "gentest_consumer_module_meson" _meson_consumer_module_pos)
if(_meson_consumer_module_pos EQUAL -1)
  message(FATAL_ERROR "meson.build must keep the repo-local module consumer target wired.")
endif()

string(FIND "${_meson_content}" "module_codegen_backend = 'generic'" _meson_module_backend_pos)
if(_meson_module_backend_pos EQUAL -1)
  message(FATAL_ERROR "meson.build must route its repo-local module path through the generic helper backend.")
endif()

string(FIND "${_meson_content}" "'--kind', 'modules'" _meson_kind_modules_pos)
if(_meson_kind_modules_pos EQUAL -1)
  message(FATAL_ERROR "meson.build must keep the explicit modules kind on the repo-local module path.")
endif()

string(FIND "${_meson_content}" "'--mock-metadata'" _meson_mock_metadata_pos)
if(_meson_mock_metadata_pos EQUAL -1)
  message(FATAL_ERROR "meson.build module codegen must consume explicit mock metadata.")
endif()

string(FIND "${_meson_content}" "'--metadata-output'" _meson_metadata_output_pos)
if(_meson_metadata_output_pos EQUAL -1)
  message(FATAL_ERROR "meson.build module mock generation must emit metadata for the downstream module consumer.")
endif()
...
string(FIND "${_xmake_content}" "module_name = \"gentest.consumer_simple_mocks\"" _xmake_module_name_pos)
