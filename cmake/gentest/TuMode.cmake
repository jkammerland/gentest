...
function(_gentest_resolve_synthetic_cxx_standard target out_standard out_extensions)
    ...
    if(NOT _gentest_standard OR _gentest_standard MATCHES "-NOTFOUND$")
        set(_gentest_standard 0)
        get_target_property(_gentest_compile_features ${target} COMPILE_FEATURES)
        if(_gentest_compile_features AND NOT _gentest_compile_features MATCHES "-NOTFOUND$")
            foreach(_gentest_feature IN LISTS _gentest_compile_features)
                if(_gentest_feature MATCHES "^cxx_std_([0-9]+)$" AND CMAKE_MATCH_1 GREATER _gentest_standard)
                    set(_gentest_standard "${CMAKE_MATCH_1}")
                endif()
            endforeach()
        endif()
    endif()
...
function(_gentest_append_cxx_standard_context_arg args_var target)
    ...
    if(_gentest_extensions AND NOT CMAKE_CXX_COMPILER_FRONTEND_VARIANT STREQUAL "MSVC")
        list(APPEND _gentest_args "-std=gnu++${_gentest_standard}")
    else()
        list(APPEND _gentest_args "-std=c++${_gentest_standard}")
    endif()
