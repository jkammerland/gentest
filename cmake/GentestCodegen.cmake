...
function(gentest_attach_codegen target)
    ...
    if(_gentest_mode STREQUAL "tu")
        foreach(_tu IN LISTS _gentest_tus)
            ...
            # Keep wrapper filenames stable across source list reorderings by
            # hashing the translation unit path. This avoids renumbering all
            # wrapper files when adding/removing sources.
            set(_gentest_hash_input "${_tu}")
            if(DEFINED CMAKE_SOURCE_DIR)
                cmake_path(RELATIVE_PATH _tu BASE_DIRECTORY "${CMAKE_SOURCE_DIR}" OUTPUT_VARIABLE _gentest_rel)
                if(NOT _gentest_rel STREQUAL "")
                    set(_gentest_hash_input "${_gentest_rel}")
                endif()
                unset(_gentest_rel)
            endif()
            string(MD5 _gentest_tu_md5 "${_gentest_hash_input}")
            string(SUBSTRING "${_gentest_tu_md5}" 0 8 _gentest_tu_md5_8)
            math(EXPR _gentest_tu_id "0x${_gentest_tu_md5_8}")

            list(APPEND _gentest_wrapper_headers "${_gentest_output_dir}/tu_${_gentest_tu_id}_${_stem}.gentest.h")
            list(APPEND _gentest_wrapper_cpp "${_gentest_output_dir}/tu_${_gentest_tu_id}_${_stem}.gentest.cpp")

            unset(_gentest_hash_input)
            unset(_gentest_tu_md5)
            unset(_gentest_tu_md5_8)
            unset(_gentest_tu_id)
        endforeach()
    endif()
