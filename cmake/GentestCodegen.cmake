...
function(gentest_attach_fuzztest_codegen target)
    set(options NO_INCLUDE_SOURCES QUIET_CLANG)
    set(one_value_args OUTPUT)
    set(multi_value_args SOURCES CLANG_ARGS DEPENDS)
    cmake_parse_arguments(GENTEST "${options}" "${one_value_args}" "${multi_value_args}" ${ARGN})

    if(NOT GENTEST_SOURCES)
        message(FATAL_ERROR "gentest_attach_fuzztest_codegen requires SOURCES")
    endif()

    if(NOT GENTEST_OUTPUT)
        set(GENTEST_OUTPUT "${CMAKE_CURRENT_BINARY_DIR}/${target}_fuzz_generated.cpp")
    endif()

    # Configure-time collision checks: avoid clobbering fuzz outputs.
    if("${GENTEST_OUTPUT}" MATCHES "\\$<")
        message(WARNING
            "gentest_attach_fuzztest_codegen(${target}): OUTPUT contains generator expressions; collision checks skipped: '${GENTEST_OUTPUT}'")
    else()
        set(_gentest_output_path "${GENTEST_OUTPUT}")
        cmake_path(ABSOLUTE_PATH _gentest_output_path BASE_DIRECTORY "${CMAKE_CURRENT_BINARY_DIR}" NORMALIZE
                   OUTPUT_VARIABLE _gentest_output_abs)
        ...
        get_property(_gentest_prev_owner GLOBAL PROPERTY "GENTEST_FUZZ_CODEGEN_OUTPUT_OWNER_${_gentest_output_md5}")
        if(_gentest_prev_owner)
            if(NOT _gentest_prev_owner STREQUAL "${target}")
                message(FATAL_ERROR
                    "gentest_attach_fuzztest_codegen(${target}): OUTPUT '${_gentest_output_abs}' is already used by '${_gentest_prev_owner}'. "
                    "Each target must have a unique OUTPUT to avoid generated file clobbering.")
            endif()
            message(FATAL_ERROR
                "gentest_attach_fuzztest_codegen(${target}): OUTPUT '${_gentest_output_abs}' is registered multiple times for the same target. "
                "Call gentest_attach_fuzztest_codegen() once per target and list all SOURCES in that call.")
        endif()
        set_property(GLOBAL PROPERTY "GENTEST_FUZZ_CODEGEN_OUTPUT_OWNER_${_gentest_output_md5}" "${target}")
        ...
        foreach(_gentest_src IN LISTS GENTEST_SOURCES)
            ...
            if(_gentest_src_key STREQUAL _gentest_output_key)
                message(FATAL_ERROR
                    "gentest_attach_fuzztest_codegen(${target}): OUTPUT '${_gentest_output_abs}' would overwrite a scanned source file '${_gentest_src_abs}'.")
            endif()
        endforeach()
    endif()
    ...
    elseif(GENTEST_CODEGEN_TARGET)
        if(NOT TARGET ${GENTEST_CODEGEN_TARGET})
            message(FATAL_ERROR "gentest_attach_fuzztest_codegen: GENTEST_CODEGEN_TARGET='${GENTEST_CODEGEN_TARGET}' does not exist")
        endif()
        set(_gentest_codegen_target "${GENTEST_CODEGEN_TARGET}")
        set(_gentest_codegen_executable $<TARGET_FILE:${GENTEST_CODEGEN_TARGET}>)
    elseif(TARGET gentest_codegen)
        ...
        message(FATAL_ERROR
            "gentest_attach_fuzztest_codegen: no gentest code generator available. "
            "Either enable -DGENTEST_BUILD_CODEGEN=ON (native builds) or provide a host tool via "
            "-DGENTEST_CODEGEN_EXECUTABLE=<path> (cross builds).")
    endif()
    ...
    set(_gentest_custom_command_args
        ...
        COMMENT "Generating ${GENTEST_OUTPUT} for target ${target} (fuzztest)"
        VERBATIM)
    if(POLICY CMP0171)
        list(APPEND _gentest_custom_command_args CODEGEN)
    endif()
