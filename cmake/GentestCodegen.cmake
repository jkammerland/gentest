...
function(_gentest_pp_render_condition_expr expr depth out_expr out_known)
    if(${depth} GREATER 16)
        set(${out_expr} "" PARENT_SCOPE)
        set(${out_known} FALSE PARENT_SCOPE)
        return()
    endif()
    ...
    set(_gentest_rendered "")
    set(_gentest_idx 0)
    while(_gentest_idx LESS _gentest_token_count)
        list(GET _gentest_tokens ${_gentest_idx} _gentest_token)
        set(_gentest_emit "")

        if(_gentest_token STREQUAL "defined")
            math(EXPR _gentest_idx "${_gentest_idx} + 1")
            if(_gentest_idx GREATER_EQUAL _gentest_token_count)
                set(${out_expr} "" PARENT_SCOPE)
                set(${out_known} FALSE PARENT_SCOPE)
                return()
            endif()

            list(GET _gentest_tokens ${_gentest_idx} _gentest_next)
            ...
            if(_gentest_next_is_lparen)
                ...
                list(GET _gentest_tokens ${_gentest_idx} _gentest_ident)
                math(EXPR _gentest_idx "${_gentest_idx} + 1")
                if(_gentest_idx GREATER_EQUAL _gentest_token_count)
                    set(${out_expr} "" PARENT_SCOPE)
                    set(${out_known} FALSE PARENT_SCOPE)
                ...
                list(GET _gentest_tokens ${_gentest_idx} _gentest_close)
        ...
        elseif(_gentest_token MATCHES "^[A-Za-z_][A-Za-z0-9_]*$")
            ...
            if(DEFINED ${_gentest_macro_var})
                ...
                if(_gentest_macro_value STREQUAL "")
                    set(_gentest_emit "1")
                elseif(_gentest_macro_value MATCHES "^[+-]?(0[xX][0-9A-Fa-f]+|[0-9]+)$")
                    set(_gentest_emit "${_gentest_macro_value}")
                else()
                    math(EXPR _gentest_next_depth "${depth} + 1")
                    _gentest_pp_render_condition_expr("${_gentest_macro_value}" ${_gentest_next_depth}
                        _gentest_macro_expr _gentest_macro_known)
                    if(NOT _gentest_macro_known)
                        set(${out_expr} "" PARENT_SCOPE)
                        set(${out_known} FALSE PARENT_SCOPE)
                        return()
                    endif()
                    set(_gentest_emit "( ${_gentest_macro_expr} )")
                endif()
            else()
                set(_gentest_emit "0")
            endif()
        elseif(_gentest_token MATCHES "^[0-9][A-Za-z0-9_]*$")
            set(_gentest_emit "${_gentest_token}")
        elseif(_gentest_token STREQUAL "&&")
            set(_gentest_emit "AND")
        elseif(_gentest_token STREQUAL "||")
            set(_gentest_emit "OR")
        elseif(_gentest_token STREQUAL "!")
            set(_gentest_emit "NOT")
        elseif(_gentest_token STREQUAL "==")
            set(_gentest_emit "EQUAL")
        elseif(_gentest_token STREQUAL ">=")
            set(_gentest_emit "GREATER_EQUAL")
        elseif(_gentest_token STREQUAL "<=")
            set(_gentest_emit "LESS_EQUAL")
        elseif(_gentest_token STREQUAL ">")
            set(_gentest_emit "GREATER")
        elseif(_gentest_token STREQUAL "<")
            set(_gentest_emit "LESS")
        else()
            set(_gentest_paren_tokens "(" ")")
            list(FIND _gentest_paren_tokens "${_gentest_token}" _gentest_paren_idx)
            if(NOT _gentest_paren_idx EQUAL -1)
                set(_gentest_emit "${_gentest_token}")
            else()
                set(${out_expr} "" PARENT_SCOPE)
                set(${out_known} FALSE PARENT_SCOPE)
        ...
        if(_gentest_rendered STREQUAL "")
            set(_gentest_rendered "${_gentest_emit}")
        else()
            string(APPEND _gentest_rendered " ${_gentest_emit}")
        endif()
        math(EXPR _gentest_idx "${_gentest_idx} + 1")
    endwhile()

    set(${out_expr} "${_gentest_rendered}" PARENT_SCOPE)
    set(${out_known} TRUE PARENT_SCOPE)
endfunction()

function(_gentest_pp_eval_condition expr out_known out_value)
    _gentest_pp_render_condition_expr("${expr}" 0 _gentest_rendered _gentest_rendered_known)
    if(NOT _gentest_rendered_known OR _gentest_rendered STREQUAL "")
        set(${out_known} FALSE PARENT_SCOPE)
        set(${out_value} FALSE PARENT_SCOPE)
        return()
    endif()

    set(_gentest_eval_result FALSE)
    cmake_language(EVAL CODE
        "if(${_gentest_rendered})\n"
        "  set(_gentest_eval_result TRUE)\n"
        "else()\n"
        "  set(_gentest_eval_result FALSE)\n"
        "endif()")

    set(${out_known} TRUE PARENT_SCOPE)
    set(${out_value} "${_gentest_eval_result}" PARENT_SCOPE)
endfunction()

function(_gentest_extract_module_name input out_name)
    _gentest_try_extract_module_name("${input}" _gentest_module_name)
        ...
        if(_gentest_is_preprocessor)
            ...
            if(NOT _gentest_pp_continuation AND NOT _gentest_pp_buffer STREQUAL "")
                ...
                if(_gentest_pp_keyword STREQUAL "if")
                    ...
                    _gentest_pp_eval_condition("${_gentest_pp_rest}" _gentest_eval_known _gentest_eval_value)
...
#include \"${_wrap_header_name}\"\n\
            ...
            set(_gentest_registration_import_name "${_module_name}")
            string(FIND "${_module_name}" ":" _gentest_partition_pos)
            if(NOT _gentest_partition_pos EQUAL -1)
                # Non-module registration TUs cannot import partitions directly.
                # Route them through the primary module instead.
                string(SUBSTRING "${_module_name}" 0 ${_gentest_partition_pos} _gentest_registration_import_name)
            endif()
            set(_shim_content
"// This file is auto-generated by gentest (CMake shim).\n\
// Do not edit manually.\n\
\n\
// Pull standard/gentest headers in first so the registration TU can use the\n\
// generated registration helpers without re-entering the gentest runtime\n\
// headers after importing the named test module.\n\
#include <array>\n\
#include <memory>\n\
#include <span>\n\
#include <string>\n\
...
import gentest;\n\
\n\
// Import the named test module so exported test functions and fixture types are\n\
// visible to the generated registration code.\n\
import ${_gentest_registration_import_name};\n\
\n\
