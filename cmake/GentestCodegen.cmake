...
function(_gentest_write_explicit_module_header_view input_file output_file)
    file(READ "${input_file}" _gentest_header_view_content)
    string(REPLACE "\r\n" "\n" _gentest_header_view_content "${_gentest_header_view_content}")
    string(REPLACE "\r" "\n" _gentest_header_view_content "${_gentest_header_view_content}")

    string(REGEX REPLACE "(^|\n)[ \t]*#define[ \t]+GENTEST_NO_AUTO_MOCK_INCLUDE[^\n]*" "" _gentest_header_view_content "${_gentest_header_view_content}")
    string(REGEX REPLACE "(^|\n)[ \t]*#include[ \t]*\"gentest/mock\\.h\"[^\n]*" "" _gentest_header_view_content "${_gentest_header_view_content}")
    string(REGEX REPLACE "(^|\n)[ \t]*#undef[ \t]+GENTEST_NO_AUTO_MOCK_INCLUDE[^\n]*" "" _gentest_header_view_content "${_gentest_header_view_content}")
    string(REGEX REPLACE "(^|\n)[ \t]*module[ \t]*;[^\n]*" "" _gentest_header_view_content "${_gentest_header_view_content}")
    string(REGEX REPLACE "(^|\n)[ \t]*export[ \t]+[^;\n]*module[ \t]+[^;\n]*;[^\n]*" "" _gentest_header_view_content "${_gentest_header_view_content}")
    string(REGEX REPLACE "(^|\n)[ \t]*(export[ \t]+)?[^;\n]*import[ \t]+[^;\n]*;[^\n]*" "" _gentest_header_view_content "${_gentest_header_view_content}")
    string(REGEX REPLACE "(^|\n)([ \t]*)export([ \t]+)" "\\1\\2" _gentest_header_view_content "${_gentest_header_view_content}")
    string(REGEX REPLACE "\n{3,}" "\n\n" _gentest_header_view_content "${_gentest_header_view_content}")
    string(REGEX REPLACE "^[ \t\r\n]+" "" _gentest_header_view_content "${_gentest_header_view_content}")
    string(REGEX REPLACE "[ \t\r\n]+$" "" _gentest_header_view_content "${_gentest_header_view_content}")

    file(WRITE "${output_file}"
"// This file is auto-generated by gentest (explicit mocks header view).\n\
// Do not edit manually.\n\
\n\
#pragma once\n\
\n\
${_gentest_header_view_content}\n")
endfunction()

function(_gentest_materialize_explicit_module_mock_defs output_dir out_defs out_public_files out_header_views)
    set(_gentest_defs_dir "${output_dir}/defs")
    file(MAKE_DIRECTORY "${_gentest_defs_dir}")

    set(_gentest_materialized_defs "")
    set(_gentest_public_files "")
    set(_gentest_header_views "")
    foreach(_gentest_def IN LISTS ARGN)
        set_property(DIRECTORY APPEND PROPERTY CMAKE_CONFIGURE_DEPENDS "${_gentest_def}")
        get_filename_component(_gentest_def_name "${_gentest_def}" NAME)
        string(MD5 _gentest_def_hash "${_gentest_def}")
...
"\n#define GENTEST_NO_AUTO_MOCK_INCLUDE 1\n#include \"gentest/mock.h\"\n#undef GENTEST_NO_AUTO_MOCK_INCLUDE\n")
        ...
        set(_gentest_header_view "${_gentest_defs_dir}/${_gentest_def_hash}_${_gentest_def_name}.gentest.hpp")
        _gentest_write_explicit_module_header_view("${_gentest_materialized_def}" "${_gentest_header_view}")
        list(APPEND _gentest_materialized_defs "${_gentest_materialized_def}")
        set(_gentest_emit_header_view FALSE)
        foreach(_gentest_staged_file IN LISTS _gentest_staged_files)
            if(IS_DIRECTORY "${_gentest_staged_file}")
                continue()
            endif()
            file(READ "${_gentest_staged_file}" _gentest_staged_content)
            if(_gentest_staged_content MATCHES "gentest[ \t\r\n]*::[ \t\r\n]*mock[ \t\r\n]*<")
                set(_gentest_emit_header_view TRUE)
                break()
            endif()
        endforeach()
        if(_gentest_emit_header_view)
            list(APPEND _gentest_header_views "${_gentest_header_view}")
        endif()
        list(APPEND _gentest_public_files ${_gentest_staged_files})
    endforeach()
    ...
    set(${out_defs} "${_gentest_materialized_defs}" PARENT_SCOPE)
    set(${out_public_files} "${_gentest_public_files}" PARENT_SCOPE)
    set(${out_header_views} "${_gentest_header_views}" PARENT_SCOPE)
endfunction()
...
\n")
    ...
    if(_gentest_textual_defs OR _gentest_materialized_module_defs)
        ...
        endforeach()
        foreach(_gentest_header_view IN LISTS _gentest_module_header_views)
            file(RELATIVE_PATH _gentest_public_header_view_include "${_gentest_public_header_dir}" "${_gentest_header_view}")
            string(APPEND _gentest_public_header_content "#include \"${_gentest_public_header_view_include}\"\n")
        endforeach()
        ...
        target_sources(${target}
            PUBLIC
                FILE_SET gentest_explicit_mock_module_headers
                    TYPE HEADERS
                    BASE_DIRS "${_gentest_output_dir}/defs"
