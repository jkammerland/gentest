...
function(gentest_add_suite suite)
    ...
    if(NOT GENTEST_NO_CTEST)
        ...
        set_tests_properties(${suite} PROPERTIES LABELS "suite-${suite};default-suite;ci-compat")
    endif()
endfunction()
...
function(gentest_add_cmake_script_test)
    ...
    set(multi_value_args ARGS ENV_VARS DEFINES ENCODED_DEFINES)
    ...
    foreach(encoded_def IN LISTS GENTEST_ENCODED_DEFINES)
        string(FIND "${encoded_def}" "=" _encoded_def_eq_pos)
        if(_encoded_def_eq_pos EQUAL -1)
            message(FATAL_ERROR "gentest_add_cmake_script_test: ENCODED_DEFINES entry '${encoded_def}' must use KEY=HEXVALUE")
        endif()

        string(SUBSTRING "${encoded_def}" 0 ${_encoded_def_eq_pos} _encoded_def_name)
        if(NOT _encoded_def_name MATCHES "^[A-Za-z_][A-Za-z0-9_]*$")
            message(FATAL_ERROR "gentest_add_cmake_script_test: invalid encoded define name '${_encoded_def_name}' in '${encoded_def}'")
        endif()

        string(LENGTH "${encoded_def}" _encoded_def_len)
        math(EXPR _encoded_def_value_pos "${_encoded_def_eq_pos} + 1")
        if(_encoded_def_value_pos LESS _encoded_def_len)
            string(SUBSTRING "${encoded_def}" ${_encoded_def_value_pos} -1 _encoded_def_hex)
            string(LENGTH "${_encoded_def_hex}" _encoded_def_hex_len)
            math(EXPR _encoded_def_hex_remainder "${_encoded_def_hex_len} % 2")
            if(_encoded_def_hex_remainder OR NOT _encoded_def_hex MATCHES "^[0-9A-Fa-f]*$")
                message(FATAL_ERROR "gentest_add_cmake_script_test: encoded define '${_encoded_def_name}' has malformed hex payload")
            endif()
            unset(_encoded_def_hex_len)
            unset(_encoded_def_hex_remainder)
        else()
            set(_encoded_def_hex "")
        endif()
        if(NOT _defines_payload STREQUAL "")
            string(APPEND _defines_payload "|")
        endif()
        string(APPEND _defines_payload "${_encoded_def_name}=${_encoded_def_hex}")

        unset(_encoded_def_eq_pos)
        unset(_encoded_def_name)
        unset(_encoded_def_len)
        unset(_encoded_def_value_pos)
        unset(_encoded_def_hex)
    endforeach()
    unset(encoded_def)
...
function(gentest_add_check_counts)
    set(options LIST)
    set(one_value_args NAME PROG PASS FAIL SKIP BLOCKED XFAIL XPASS CASES EXPECT_RC)
    ...
    if(GENTEST_LIST)
        ...
        if(NOT "${GENTEST_BLOCKED}" STREQUAL "")
            list(APPEND _defines "BLOCKED=${GENTEST_BLOCKED}")
        endif()
...
function(gentest_add_check_inventory)
    set(one_value_args NAME PROG CASES PASS FAIL SKIP BLOCKED XFAIL XPASS EXPECT_RC EXPECTED_LIST_FILE)
    set(multi_value_args ARGS)
...
function(gentest_add_check_death)
    set(options NO_EMULATOR)
    set(one_value_args NAME PROG REQUIRED_SUBSTRING)
    set(multi_value_args ARGS ENV_VARS REQUIRED_SUBSTRINGS)
    cmake_parse_arguments(PARSE_ARGV 0 GENTEST "${options}" "${one_value_args}" "${multi_value_args}")
    ...
    set(_defines)
    set(_encoded_defines)
    if(NOT GENTEST_REQUIRED_SUBSTRING STREQUAL "")
        string(REPLACE "\\;" ";" _required_substring_value "${GENTEST_REQUIRED_SUBSTRING}")
        string(HEX "${_required_substring_value}" _required_substring_hex)
        list(APPEND _encoded_defines "REQUIRED_SUBSTRING=${_required_substring_hex}")
        unset(_required_substring_value)
        unset(_required_substring_hex)
    endif()
    if(GENTEST_REQUIRED_SUBSTRINGS)
        list(LENGTH GENTEST_REQUIRED_SUBSTRINGS _required_substrings_count)
        list(APPEND _defines "REQUIRED_SUBSTRINGS_COUNT=${_required_substrings_count}")
        set(_required_substring_index 0)
        foreach(_required_substring IN LISTS GENTEST_REQUIRED_SUBSTRINGS)
            string(REPLACE "\\;" ";" _required_substring_value "${_required_substring}")
            string(HEX "${_required_substring_value}" _required_substring_hex)
            list(APPEND _encoded_defines "REQUIRED_SUBSTRING_${_required_substring_index}=${_required_substring_hex}")
            math(EXPR _required_substring_index "${_required_substring_index} + 1")
        endforeach()
        unset(_required_substrings_count)
        unset(_required_substring_index)
        unset(_required_substring)
        unset(_required_substring_value)
        unset(_required_substring_hex)
    endif()
    ...
    gentest_add_cmake_script_test(
        ...
        ${_no_emulator_arg}
        ENCODED_DEFINES ${_encoded_defines}
        DEFINES ${_defines})
endfunction()
