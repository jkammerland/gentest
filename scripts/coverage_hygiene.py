...
def main() -> int:
    ...
    ap.add_argument("--roots", nargs="*", default=["src", "tools/src"], help="Relative source roots to inspect")
    ap.add_argument("--exclude-prefix", nargs="*", default=[], help="Path prefixes to skip")
    ap.add_argument(
        "--intentional",
        nargs="*",
        default=sorted(INTENTIONAL_SOURCES),
        help="Path prefixes considered intentional placeholders (not coverage-failing).",
    )
    ap.add_argument(
        "--no-exec",
        nargs="*",
        default=sorted(NO_EXEC_SOURCES),
