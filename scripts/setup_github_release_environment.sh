...
resolve_fingerprint() {
  ...
  if [[ -n "${selector}" ]]; then
    while IFS= read -r fingerprint; do
      ...
      "${gpg_command}" --batch --with-colons --list-secret-keys "${selector}" 2>/dev/null |
    ...
    while IFS= read -r fingerprint; do
      ...
      "${gpg_command}" --batch --with-colons --list-secret-keys 2>/dev/null |
  ...
  if [[ -z "${selector}" ]] && ((fingerprint_count != 1)); then
    die "found ${fingerprint_count} GPG secret keys; select one with --key or RELEASE_GPG_FINGERPRINT"
  fi
...
gh_command="${GENTEST_GH_COMMAND:-gh}"
gpg_command="${GENTEST_GPG_COMMAND:-gpg}"
command_is_available "${gh_command}" || die "GitHub CLI (gh) is not installed"
command_is_available "${gpg_command}" || die "GnuPG (gpg) is not installed"
"${gh_command}" auth status >/dev/null 2>&1 || die "GitHub CLI is not authenticated; run 'gh auth login' as $(id -un)"
...
for input in "${repo_inputs[@]}"; do
  ...
  canonical_repo="$("${gh_command}" repo view "${repo}" --json nameWithOwner --jq .nameWithOwner 2>/dev/null)" ||
  ...
  repo_already_added=false
  if ((repo_count > 0)); then
    for existing_repo in "${repos[@]}"; do
      if [[ "${existing_repo}" == "${canonical_repo}" ]]; then
        repo_already_added=true
        break
      fi
    done
  fi
  if [[ "${repo_already_added}" == false ]]; then
...
if [[ -n "${passphrase}" ]]; then
  private_key="$("${gpg_command}" --batch --yes --pinentry-mode loopback --passphrase-fd 3 \
    --armor --export-secret-keys "${fingerprint}" 3<<<"${passphrase}")"
else
  private_key="$("${gpg_command}" --batch --armor --export-secret-keys "${fingerprint}")"
fi
...
for repo in "${repos[@]}"; do
  ...
  "${gh_command}" api --method PUT "repos/${repo}/environments/${environment_name}" >/dev/null
  "${gh_command}" variable set GPG_FINGERPRINT \
    --repo "${repo}" \
    --env "${environment_name}" \
    --body "${fingerprint}"
  printf '%s\n' "${private_key}" |
    "${gh_command}" secret set GPG_PRIVATE_KEY \
      --repo "${repo}" \
      --env "${environment_name}"
  if [[ -n "${passphrase}" ]]; then
    printf '%s' "${passphrase}" |
      "${gh_command}" secret set GPG_PASSPHRASE \
        --repo "${repo}" \
        --env "${environment_name}"
  fi

  echo "Configured ${repo}/${environment_name}:"
  "${gh_command}" variable list --repo "${repo}" --env "${environment_name}" |
    awk '$1 == "GPG_FINGERPRINT" { print "  variable: " $1 "=" $2 }'
  "${gh_command}" secret list --repo "${repo}" --env "${environment_name}" |
    awk '$1 ~ /^GPG_/ { print "  secret:   " $1 }'
done
