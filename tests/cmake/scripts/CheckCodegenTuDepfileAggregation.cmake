...
function(_gentest_run_tu_codegen depfile_path out_rc out_out out_err)
  ...
  if(DEFINED _gentest_run_parse_cache_dir AND NOT "${_gentest_run_parse_cache_dir}" STREQUAL "")
    list(APPEND _extra_codegen_args --parse-cache-dir "${_gentest_run_parse_cache_dir}")
  endif()
  set(_timeout_args)
...
elseif(_mode STREQUAL "timing_json")
  ...
  set(_gentest_run_parse_cache_dir "${_work_dir}/parse-cache")
  ...
  foreach(_phase_index RANGE 0 ${_phase_count})
    ...
    if(_phase_name STREQUAL "parse")
      ...
      string(JSON _cache_type TYPE "${_timing_text}" phases ${_phase_index} cache)
      string(JSON _cache_value GET "${_timing_text}" phases ${_phase_index} cache)
      if(NOT _cache_type STREQUAL "STRING" OR NOT _cache_value STREQUAL "miss")
        message(FATAL_ERROR "Initial cache-enabled parse record must report cache=miss:\n${_timing_text}")
      endif()
    endif()
  ...
  endforeach()
  file(READ "${_gentest_run_timing_json}" _parallel_timing_text)
  string(REGEX MATCHALL "\\\"cache\\\"[ \t]*:[ \t]*\\\"hit\\\"" _parallel_cache_hits "${_parallel_timing_text}")
  list(LENGTH _parallel_cache_hits _parallel_cache_hit_count)
  if(NOT _parallel_cache_hit_count EQUAL 2)
    message(FATAL_ERROR "Expected two parallel parse-cache hits. Timing JSON:\n${_parallel_timing_text}")
  endif()

  # Keep one TU warm while invalidating only the other. The merge/emission
  # phase must remain byte-identical when it combines a cached result with a
  # freshly parsed result.
  file(APPEND "${_a_hpp}" "// cache invalidation comment\n")
  set(_gentest_run_timing_json "${_work_dir}/timing_mixed.json")
  _gentest_run_tu_codegen("${_depfile}" _rc _out _err)
  if(NOT _rc EQUAL 0)
    message(FATAL_ERROR
      "mixed cache-hit/miss parallel gentest_codegen failed. Output:\n${_out}\nErrors:\n${_err}")
  endif()
  foreach(_artifact IN LISTS _generated_artifacts)
    get_filename_component(_artifact_name "${_artifact}" NAME)
    file(SHA256 "${_artifact}" _mixed_hash)
    file(SHA256 "${_serial_dir}/${_artifact_name}" _serial_hash)
    if(NOT _mixed_hash STREQUAL _serial_hash)
      message(FATAL_ERROR "Mixed cache-hit/miss generated artifact differs: '${_artifact_name}'")
    endif()
  endforeach()
  file(READ "${_gentest_run_timing_json}" _mixed_timing_text)
  string(REGEX MATCHALL "\\\"cache\\\"[ \t]*:[ \t]*\\\"hit\\\"" _mixed_cache_hits "${_mixed_timing_text}")
  string(REGEX MATCHALL "\\\"cache\\\"[ \t]*:[ \t]*\\\"miss\\\"" _mixed_cache_misses "${_mixed_timing_text}")
  list(LENGTH _mixed_cache_hits _mixed_cache_hit_count)
  list(LENGTH _mixed_cache_misses _mixed_cache_miss_count)
  if(NOT _mixed_cache_hit_count EQUAL 1 OR NOT _mixed_cache_miss_count EQUAL 1)
    message(FATAL_ERROR "Expected one mixed cache hit and one miss. Timing JSON:\n${_mixed_timing_text}")
  endif()
