...
if(NOT DEFINED SOURCE_DIR OR "${SOURCE_DIR}" STREQUAL "")
  message(FATAL_ERROR "CheckTidyContextLease.cmake: SOURCE_DIR not set")
endif()
if(NOT DEFINED BUILD_ROOT OR "${BUILD_ROOT}" STREQUAL "")
  message(FATAL_ERROR "CheckTidyContextLease.cmake: BUILD_ROOT not set")
endif()
if(NOT DEFINED GENTEST_SOURCE_DIR OR "${GENTEST_SOURCE_DIR}" STREQUAL "")
  message(FATAL_ERROR "CheckTidyContextLease.cmake: GENTEST_SOURCE_DIR not set")
endif()
...
set(_work_dir "${BUILD_ROOT}/tidy_context_lease")
...
if(NOT _ninja)
  gentest_skip_test("tidy context lease regression: ${_ninja_reason}")
  return()
endif()

gentest_resolve_clang_fixture_compilers(_clang _clangxx)
if(NOT _clang OR NOT _clangxx)
  gentest_skip_test("tidy context lease regression: clang/clang++ not found")
  return()
endif()
...
if(NOT _clang_tidy)
  gentest_skip_test("tidy context lease regression: clang-tidy not found next to selected clang toolchain")
  return()
endif()
...
if(NOT "${_clangxx_major}" STREQUAL "" AND NOT "${_clang_tidy_major}" STREQUAL "" AND NOT "${_clangxx_major}" STREQUAL "${_clang_tidy_major}")
  gentest_skip_test(
    "tidy context lease regression: clang-tidy major ${_clang_tidy_major} does not match selected clang++ major ${_clangxx_major}")
  return()
endif()
...
if(NOT EXISTS "${_plugin_ninja}")
  gentest_skip_test("tidy context lease regression: plugin build did not generate Ninja files")
  return()
endif()
...
if(_plugin_target_pos EQUAL -1)
  gentest_skip_test("tidy context lease regression: clang-tidy plugin target is unavailable")
  return()
endif()
...
message(STATUS "Configure tidy context lease source fixture...")
gentest_check_run_or_fail(
  COMMAND
    "${CMAKE_COMMAND}"
    -G Ninja
...
string(FIND "${_list_tidy_text}" "gentest-context-lease" _loaded_check_pos)
if(NOT _list_tidy_rc EQUAL 0 OR _loaded_check_pos EQUAL -1)
  gentest_skip_test("tidy context lease regression: clang-tidy did not expose loaded gentest plugin checks")
  return()
endif()

message(STATUS "Run tidy context lease negative fixture...")
execute_process(
  COMMAND
    "${_clang_tidy}"
    ${_tidy_common_args}
    "--warnings-as-errors=gentest-context-lease"
...
if(_bad_tidy_rc EQUAL 0)
  message(FATAL_ERROR "Expected gentest-context-lease diagnostics for bad_cases.cpp, but clang-tidy succeeded:\n${_bad_tidy_text}")
endif()
...
if(NOT _good_tidy_rc EQUAL 0)
  message(FATAL_ERROR
    "Expected no gentest-context-lease diagnostics for good_cases.cpp.\n"
    "stdout:\n${_good_tidy_out}\n"
    "stderr:\n${_good_tidy_err}")
endif()

message(STATUS "Observed gentest-context-lease tidy diagnostics")
