...
constexpr std::string_view kFixtureName = "regressions::AssertingTeardownFixture";

std::shared_ptr<void> create_fixture(std::string_view, std::string &) { return std::make_shared<int>(1); }

void teardown_assert(void *, std::string &) { gentest::asserts::ASSERT_TRUE(false, "manual-teardown-assert"); }
...
gentest::Case kCases[] = {
    {
        .name             = "regressions/shared_fixture_manual_teardown_assert_exit/smoke",
        .fn               = &smoke,
        .file             = __FILE__,
        .line             = kSmokeLine,
        .is_benchmark     = false,
