...
gentest::async_test<void> interleaved_first_fatal(FirstInterleavedFx &) {
    release_first.reset();
    keep_second_suspended.reset();
    first_waiting.set();
    co_await release_first.wait("second interleaved case did not release first");
...
gentest::async_test<void> interleaved_second_holds_hook(SecondInterleavedFx &) {
    co_await first_waiting.wait("first interleaved case did not suspend");
    release_first.set();
...
gentest::async_test<void> plain_interleaved_fatal() {
    release_plain.reset();
    keep_fixture_suspended.reset();
    plain_waiting.set();
    co_await release_plain.wait("fixture case did not release plain fatal case");
...
gentest::async_test<void> plain_interleaved_fixture_holds_hook(SecondInterleavedFx &) {
    co_await plain_waiting.wait("plain fatal case did not suspend");
    release_plain.set();
