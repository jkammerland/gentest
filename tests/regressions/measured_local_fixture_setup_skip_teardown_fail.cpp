...
struct BenchSkipThenTeardownFailFixture : gentest::FixtureSetup, gentest::FixtureTearDown {
    void setUp() override {
        g_bench_setup_entered.store(true, std::memory_order_relaxed);
        gentest::skip("bench-setup-skip-marker");
    }
    void tearDown() override {
        g_bench_teardown_count.fetch_add(1, std::memory_order_relaxed);
        throw std::runtime_error("bench-teardown-fail-marker");
    }
};

struct JitterSkipThenTeardownFailFixture : gentest::FixtureSetup, gentest::FixtureTearDown {
    void setUp() override {
        g_jitter_setup_entered.store(true, std::memory_order_relaxed);
        gentest::skip("jitter-setup-skip-marker");
    }
    void tearDown() override {
        g_jitter_teardown_count.fetch_add(1, std::memory_order_relaxed);
        throw std::runtime_error("jitter-teardown-fail-marker");
    }
};
...
void               bench_setup_skip_teardown_fail(void *) {
    ...
    if (phase != gentest::detail::BenchPhase::None) {
        struct BenchState {
            gentest::detail::FixtureHandle<BenchSkipThenTeardownFailFixture> fx{
                gentest::detail::FixtureHandle<BenchSkipThenTeardownFailFixture>::empty()};
        ...
        if (phase == gentest::detail::BenchPhase::Call) {
            ...
            gentest::asserts::EXPECT_TRUE(false, "regression marker: bench call executed after setup skip");
            return;
        }
        return;
    }
...
void               jitter_setup_skip_teardown_fail(void *) {
    ...
    if (phase != gentest::detail::BenchPhase::None) {
        struct BenchState {
            gentest::detail::FixtureHandle<JitterSkipThenTeardownFailFixture> fx{
                gentest::detail::FixtureHandle<JitterSkipThenTeardownFailFixture>::empty()};
            bool teardown_armed = false;
        };
        static thread_local BenchState bench_state{};
        ...
        if (phase == gentest::detail::BenchPhase::Call) {
            ...
            gentest::asserts::EXPECT_TRUE(false, "regression marker: jitter call executed after setup skip");
            return;
        }
        return;
    }
...
constexpr std::string_view kBenchCaseName  = "regressions/measured_local_fixture_setup_skip_teardown_fail/bench";
constexpr std::string_view kJitterCaseName = "regressions/measured_local_fixture_setup_skip_teardown_fail/jitter";
