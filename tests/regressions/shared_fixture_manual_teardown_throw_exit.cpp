...
constexpr std::string_view kFixtureName = "regressions::ThrowingTeardownFixture";

std::shared_ptr<void> create_fixture(std::string_view, std::string &) { return std::make_shared<int>(1); }

void teardown_throw(void *, std::string &) { throw std::runtime_error("manual-teardown-throw"); }

void smoke(void *) {}

gentest::Case kCases[] = {
    {
        .name             = "regressions/shared_fixture_manual_teardown_throw_exit/smoke",
        .fn               = &smoke,
        .file             = __FILE__,
        .line             = 12,
        .is_benchmark     = false,
