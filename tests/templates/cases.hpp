...
inline void emplace_matrix(bool use_list) {
    if (use_list) {
        std::list<T> values;
...
inline void typed_values(int v) {
...
inline void mix_type_nttp_value(int v) {
    if constexpr (!std::is_same_v<T, int>) {
...
inline void multi_mixed_split(const std::string &s) {
