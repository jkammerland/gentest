...
local function ensure_fallback_compdb(filepath, sources, defines, include_dirs, clang_args, forced_includes, runtime_os, runtime_io)
    ...
    for _, source in ipairs(sources or {}) do
        local arguments = {"clang++", "-std=c++20", "-DGENTEST_CODEGEN=1", "-DFMT_HEADER_ONLY"}
        for _, define in ipairs(defines or {}) do
            table.insert(arguments, "-D" .. define)
        end
        for _, include_dir in ipairs(include_dirs or {}) do
            table.insert(arguments, "-I" .. project_path(include_dir))
        end
        for _, arg in ipairs(clang_args or {}) do
            table.insert(arguments, arg)
        end
        for _, include_file in ipairs(forced_includes or {}) do
            table.insert(arguments, "-include")
            table.insert(arguments, include_file)
        end
        ...
        for _, argument in ipairs(arguments) do
            table.insert(encoded_arguments, json_string(argument))
        end
        table.insert(entries, "  {\"directory\": " .. json_string(project_root()) .. ", \"file\": " .. json_string(source) ..
            ", \"arguments\": [" .. table.concat(encoded_arguments, ", ") .. "]}")
    end
...
local function ensure_textual_mock_anchor(filepath, public_header, runtime_os, runtime_io)
    local output_dir = path.directory(filepath)
    if output_dir and output_dir ~= "" then
        runtime_os.mkdir(output_dir)
    end

    local include_path = path.relative(project_path(public_header), project_path(output_dir)):gsub("\\", "/")
    local contents = "// This file is auto-generated by gentest's Xmake helper.\n" ..
...
local function resolve_codegen()
    ...
    if explicit_codegen then
        return explicit_codegen, nil, nil
    end
...
function gentest_add_mocks(opts)
    ...
    on_config(function ()
        ...
        if config.aggregate_source then
            ...
            ensure_textual_mock_anchor(config.anchor_source, public_header, os, io)
        end
    end)
...
function gentest_attach_codegen(opts)
    ...
    local registration_d = path.join(output_dir, sanitize_target_id(target_name) .. ".gentest.d")
    local fallback_compdb_dir, fallback_compdb_file = fallback_compdb_paths(output_dir)
    ...
    local config = {
        ...
        headerfiles = opts.headerfiles or {},
    }
    if kind == "modules" then
    ...
    if #config.headerfiles > 0 then
        add_headerfiles(table.unpack(config.headerfiles))
    end
