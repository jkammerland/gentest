...
using TimedRunStatus         = gentest::runner::TimedRunStatus;
using MeasurementCaseFailure = gentest::runner::MeasurementCaseFailure;
...
auto run_all_tests(std::span<const char *> args) -> int {
    ...
    if (!fixture_runtime_blocked && !(opt.fail_fast && tests_stopped)) {
        bench_status = gentest::runner::run_selected_benches(
            ...
            [&](const Case &measured, const gentest::runner::BenchResult &result) {
    ...
    if (!fixture_runtime_blocked && !(opt.fail_fast && (tests_stopped || bench_status.stopped))) {
        jitter_status = gentest::runner::run_selected_jitters(
            ...
            [&](const Case &measured, const gentest::runner::JitterResult &result) {
                record_measured_success_report_item(state, measured, result);
            },
            [&](const Case &measured, const MeasurementCaseFailure &failure, std::string_view failure_message) {
                record_measured_failure_summary(state, measured, failure, failure_message);
