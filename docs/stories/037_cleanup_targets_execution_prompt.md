...
| High | Module-wrapper mock injection | Finish same-module registration and split mock phases, then remove source transformation, namespace close/reopen hacks, `--module-wrapper-output`, and related CMake/Xmake plumbing. | Module tests register natively from generated artifacts without final include `.cpp` hacks. Mock extraction and mock rendering can run as separate codegen phases. |
