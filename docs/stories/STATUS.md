...
| `035` | replace module-wrapper mock injection with split mock registration | `Open` | story created from `034` closure split | split mock protocol still does not replace integrated module-wrapper mock attachment injection or support module-owned mock injection in same-module registration |
