...
| `023` | reduce installed public API leakage of runtime internals | `Partial` | `3a10cf87`, `bcba719a`, `pending-commit` | `context.h` is narrower and raw `SharedFixtureRegistration` is removed from the installed surface, but `registry.h` still exposes unstable fixture callbacks and `fixture.h` / `FixtureHandle` remain the larger open leak |
