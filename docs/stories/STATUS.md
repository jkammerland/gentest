...
| `031` | preserve explicit scan-deps mode in installed public-module consumers | `Rejected` | `7bcfc833`, `2026-04-14` targeted Linux rerun, `2026-04-14` native Windows rerun, `ddb4702e` | historical policy removed: named-module codegen requires `clang-scan-deps`, while textual registration never invokes it; no user-selectable mode remains |
...
| `036` | additive textual header-declaration registration | `Superseded / implemented` | issue #107 superseded the 2026-04-21 opt-in rejection on `2026-08-13`; ordinary textual targets retain authored sources and append deterministic registration slots discovered from headers | issue #115 removes the previous-major ordinary wrapper backend; named modules use `MODULE_REGISTRATION FILE_SET`, while explicit module mocks retain separate generated-mock internals |
| `037` | codegen contract cleanup campaign | `Done at ordinary-registration scope` | issue #115 migrates repository suites plus CMake, Meson, Xmake, and Bazel adapters to additive registration; removes ordinary textual wrapper rendering/owner manifests; and simplifies clang-tidy to direct authored compile commands | generated mocks and their explicit named-module provider internals remain intentionally separate |
