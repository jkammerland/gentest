...
| `037` | codegen contract cleanup campaign | `Open` | story created `2026-04-21` to own the deletion schedule promised in `034` closure; opening docs slice adds root [`DEPRECATIONS.md`](../../DEPRECATIONS.md) and links it from `README.md`, `docs/index.md`, and this status matrix; current slice adds `NO_INCLUDE_SOURCES` warning coverage, `EXPECT_SUBSTRING` warning coverage, package install absence coverage for legacy `scan_inspector` helpers, and the `036` rejection update | `EXPECT_SUBSTRING` hard-error coverage waits for the documented `2.0.0` removal target; wave 1 waits for `033`, wave 2 waits for `015`, and wave 3 waits for the release window plus `015` migration evidence |
