...
| `037` | codegen contract cleanup campaign | `Open` | story created `2026-04-21` to own the deletion schedule promised in `034` closure; opening docs slice adds root [`DEPRECATIONS.md`](../../DEPRECATIONS.md) and links it from `README.md`, `docs/index.md`, and this status matrix; current `2.0.0` slice removes legacy manifest mode (`OUTPUT`, CLI `--output`, CLI `--template`, `NO_INCLUDE_SOURCES` / `GENTEST_NO_INCLUDE_SOURCES`) and removes the top-level `EXPECT_SUBSTRING` alias, with hard-error regressions and migrated direct codegen tests | wave 1 CMake source-inspection/scan helper deletion remains; wave 2 waits for `015`; user-facing deprecation removals for wave 3 / wave 4 are landed on this branch |
