...
| `034` | make codegen own artifact planning and same-module registration | `Done` | story created `2026-04-17`; first slice in `b10252fd`, `a74ad0af`, `ad2f6b52`, `684a71a3`; current branch commits `0606b042` and `aa35d0c8`; textual wrapper manifest, same-module registration manifests, mock manifest split, named mock phase subcommands, manifest-declared named-module mock output domains, stable `gentest.artifact_manifest.v1` / `gentest.mock_manifest.v1` schemas, direct textual protocol invocation coverage, and non-CMake composition docs implemented on this branch | declaration-only textual registration was rejected in `036`; full non-CMake parity remains in `015` |
