...
| `034` | make codegen own artifact planning and same-module registration | `Done` | story created `2026-04-17`; first slice in `b10252fd`, `a74ad0af`, `ad2f6b52`, `684a71a3`; current branch commits `0606b042` and `aa35d0c8`; textual wrapper manifest, same-module registration manifests, mock manifest split, named mock phase subcommands, manifest-declared named-module mock output domains, stable `gentest.artifact_manifest.v1` / `gentest.mock_manifest.v1` schemas, direct textual protocol invocation coverage, and non-CMake composition docs implemented on this branch | closed only for the implemented first slice; integrated module-wrapper mock attachment injection replacement moved to `035`, declaration-only textual registration moved to `036`, and full non-CMake parity remains in `015` |
| `035` | replace module-wrapper mock injection with split mock registration | `Partial` | story created from `034` closure split; `b3a66a6b` wires same-module registration to consume a split mock registration manifest from a tool-owned `inspect-mocks` phase | legacy module-wrapper source transformation and related non-registration compatibility plumbing still need removal after wrapper-mode mock users migrate |
