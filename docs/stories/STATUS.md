...
| `034` | make codegen own artifact planning and same-module registration | `Partial` | story created `2026-04-17`; first slice in `b10252fd`, `a74ad0af`, `ad2f6b52`, `684a71a3`; textual wrapper manifest, mock manifest split, named mock phase subcommands, manifest-declared named-module mock output domains, stable `gentest.artifact_manifest.v1` / `gentest.mock_manifest.v1` schemas, direct textual protocol invocation coverage, and non-CMake composition docs implemented on this branch | split mock protocol does not yet replace integrated module-wrapper mock attachment injection; optional declaration-only textual mode and full non-CMake parity remain future work |
