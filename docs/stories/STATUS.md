...
| `031` | preserve explicit scan-deps mode in installed public-module consumers | `Done` | `7bcfc833`, `2026-04-14` targeted Linux rerun, `2026-04-14` native Windows rerun, `pending-commit` | closure slice updates the regression harness to follow the actual generated per-TU output set and broader Windows launcher forms while still asserting explicit mode propagation |
