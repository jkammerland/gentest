...
| `037` | codegen contract cleanup campaign | `Open` | story created `2026-04-21` to own the deletion schedule promised in `034` closure; opening docs slice adds root [`DEPRECATIONS.md`](../../DEPRECATIONS.md) and links it from `README.md`, `docs/index.md`, and this status matrix; current slice adds `NO_INCLUDE_SOURCES` warning coverage, `EXPECT_SUBSTRING` warning coverage, package install absence coverage for legacy `scan_inspector` helpers, the `036` rejection update, and the `033` modularization gate closure | wave 1 is now unblocked by `033`; wave 2 waits for `015`; wave 3 is being handled on this branch as the `2.0.0` removal branch after the documented warning coverage landed |
