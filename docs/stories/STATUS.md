...
| `037` | codegen contract cleanup campaign | `Open` | story created `2026-04-21` to own the deletion schedule promised in `034` closure; opening docs slice adds root [`DEPRECATIONS.md`](../../DEPRECATIONS.md) and links it from `README.md`, `docs/index.md`, and this status matrix; current slice adds `NO_INCLUDE_SOURCES` warning coverage, `EXPECT_SUBSTRING` warning coverage, and package install absence coverage for legacy `scan_inspector` helpers | `EXPECT_SUBSTRING` hard-error coverage waits for the documented `2.0.0` removal target; waves 1-3 (configure-time source inspector probe; lua/bazel wrapper collapse + `.in` template removal; legacy manifest mode) remain gated by the owning story closures and release window |
