...
- Use `--kind` to scope `--run` / `--filter` to `test|bench|jitter` (default `all`).
- Run benches:
  - Exact: `--run=suite/name --kind=bench`
  - Filter: `--filter=pattern --kind=bench` (supports `*` and `?`)
...
- Run:
  - Exact: `--run=suite/name --kind=jitter`
  - Filter: `--filter=pattern --kind=jitter`
- Options:
...
- CLI examples
  ...
  - Run exact: `--run=n1/n2/my_case<int>(42)`
