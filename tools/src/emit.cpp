...
std::string additive_header_include(const CollectorOptions &opts, std::size_t scan_index, const fs::path &registration_output,
                                    const fs::path &scan_source, const fs::path &header) {
    ...
    if (scan_index < opts.compile_context_ids.size()) {
        ...
        if (const auto separator = context_id.rfind(':'); separator != std::string::npos) {
            ...
            if (!context_parent.empty() && context_parent != "." && !context_source.is_absolute()) {
                const std::string context_marker = context_parent + "/";
                if (const auto marker = normalized_header.rfind(context_marker); marker != std::string::npos) {
                    return include_literal_for_path(normalized_header.substr(marker));
                }
            }
        }
    }

    if (opts.source_root.has_value() && !opts.source_root->empty() && opts.source_root->is_relative()) {
        std::error_code ec;
        fs::path        relative = fs::relative(header, *opts.source_root, ec);
        if (!ec && !relative.empty() && *relative.begin() != "..") {
            return include_literal_for_path(relative);
        }

        // Hermetic build sandboxes may expose the repository through a different
        // absolute mount while preserving the source-root directory name. Recover
        // the stable repository-relative suffix instead of leaking that mount into
        // a generated include.
        const std::string root_marker = "/" + opts.source_root->filename().generic_string() + "/";
        if (const auto marker = normalized_header.rfind(root_marker); marker != std::string::npos) {
            return include_literal_for_path(normalized_header.substr(marker + root_marker.size()));
        }

        const std::string source_parent = scan_source.parent_path().lexically_normal().generic_string();
        if (!source_parent.empty() && source_parent != ".") {
            const std::string parent_marker = "/" + source_parent + "/";
            if (const auto marker = normalized_header.rfind(parent_marker); marker != std::string::npos) {
                return include_literal_for_path(normalized_header.substr(marker + 1));
            }
        }
    }
    return include_literal_relative_to(registration_output, header);
}
...
std::string render_textual_mock_public_header(const TextualMockPublicHeaderRenderSpec &spec) {
    const std::string defs_include =
        spec.include_definitions_source ? include_literal_for_path(spec.definitions_source_path) : std::string{};
...
std::string render_module_importer_registration_source(std::string_view module_name, std::string_view registration_header_name,
                                                       bool has_registrations, bool needs_full_registration_support) {
    std::string rendered;
    rendered.reserve(module_name.size() + registration_header_name.size() + 640);
    ...
    if (has_registrations) {
        ...
        rendered.append("// No gentest registrations were discovered for this module interface.\n");
    }
    return rendered;
}
...
bool write_module_registration_manifest(const CollectorOptions &opts) {
    ...
    for (std::size_t idx = 0; idx < opts.sources.size(); ++idx) {
        ...
        fmt::format_to(std::back_inserter(manifest),
                       ...
                       render::escape_string(module_name), render::escape_string(source_path.generic_string()),
                       render::escape_string(context_id), render::escape_string(include_dir),
                       render::escape_string(header_output.generic_string()),
...
bool validate_generated_artifact_outputs(const CollectorOptions &opts, std::string &error) {
    ...
    inputs.reserve(opts.sources.size() + opts.explicit_module_sources_by_name.size() + 1);
    ...
    if ((!opts.tu_output_headers.empty() || !opts.textual_registration_outputs.empty() || !opts.module_wrapper_outputs.empty() ||
    ...
    if (!opts.tu_output_dir.empty()) {
        ...
        parallel_for(opts.sources.size(), jobs, [&](std::size_t idx) {
                ...
                if (tu_cases.empty() && tu_fixtures.empty()) {
                    ...
                    for (const auto &header : headers_it->second) {
                        fmt::format_to(std::back_inserter(registration_source), "#include \"{}\"\n",
                                       additive_header_include(opts, idx, registration_out, source_path, header));
                    }
                    registration_source += "\n";
            ...
            if (tu_cases.empty() && tu_fixtures.empty()) {
                ...
                // Registration header consumed by the generated named-module
                // mock provider path.
            ...
            if (is_module_interface_source(opts, source_path)) {
                ...
                if (!opts.module_registration_outputs.empty()) {
                    ...
                    if (!source_mocks.empty()) {
                        log_err("gentest_codegen: MODULE_REGISTRATION cannot generate direct mocks owned by named module '{}'; "
                                "export a mock-provider API from the module and attach its generated implementation explicitly\n",
                                module_it->second);
                        statuses[idx] = 1;
                        return;
                    }
                    ...
                    const std::string registration_source = render_module_importer_registration_source(
                        module_it->second, header_out.filename().string(), !tu_cases.empty() || !tu_fixtures.empty(),
                        needs_full_registration_support);
                    if (!write_file_atomic_if_changed(module_registration_out, registration_source)) {
                        statuses[idx] = 1;
                    }
                } else {
