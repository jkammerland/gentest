...
fs::path resolve_module_registration_output(const CollectorOptions &opts, std::size_t idx) { return opts.module_registration_outputs[idx]; }
...
fs::path textual_wrapper_manifest_path(const CollectorOptions &opts, std::size_t idx) {
    ...
    return fs::path{opts.sources[idx]};
}

std::string compile_context_id_for(const CollectorOptions &opts, std::size_t idx) {
    if (idx < opts.compile_context_ids.size() && !opts.compile_context_ids[idx].empty()) {
        return opts.compile_context_ids[idx];
    }
    return idx < opts.sources.size() ? opts.sources[idx] : std::string{};
}
...
std::string render_module_registration_support_include_block(bool needs_full_registration_support) {
    const std::string_view preamble = needs_full_registration_support ? tpl::registration_preamble_full : tpl::registration_preamble_light;
    std::string            out;
    out.reserve(preamble.size() + 96);
...
std::string include_literal_for_path(fs::path path) {
    path                = path.lexically_normal();
    std::string literal = path.generic_string();
    for (auto &ch : literal) {
        if (ch == '\\') {
            ch = '/';
        }
...
std::string include_literal_relative_to(const fs::path &including_file, const fs::path &included_file) {
    ...
    return include_literal_for_path(included_file);
}

struct TextualWrapperRenderSpec {
    fs::path included_source_path;
    fs::path wrapper_output_path;
    fs::path generated_header_path;
};

struct TextualMockPublicHeaderRenderSpec {
    fs::path    definitions_source_path;
    fs::path    public_header_output_path;
    fs::path    registry_header_output_path;
    fs::path    implementation_header_output_path;
    std::string mock_api_header;
    bool        include_definitions_source          = true;
    bool        suppress_gentest_auto_mock_include  = true;
    bool        suppress_gentest_expect_call_macros = false;
};

std::string render_textual_suite_wrapper_source(const TextualWrapperRenderSpec &spec) {
    const std::string source_include = include_literal_relative_to(spec.wrapper_output_path, spec.included_source_path);
    const std::string header_include = include_literal_relative_to(spec.wrapper_output_path, spec.generated_header_path);
    return fmt::format("// This file is auto-generated by gentest_codegen.\n"
                       ...
                       "#include \"{}\"\n"
                       "#endif\n",
                       source_include, header_include, header_include);
}

std::string render_textual_mock_defs_wrapper_source(const TextualWrapperRenderSpec &spec) {
    const std::string defs_include   = include_literal_relative_to(spec.wrapper_output_path, spec.included_source_path);
    const std::string header_include = include_literal_relative_to(spec.wrapper_output_path, spec.generated_header_path);
    return fmt::format("// This file is auto-generated by gentest_codegen.\n"
                       "// Do not edit manually.\n\n"
                       ...
                       "#include \"{}\"\n"
                       "#endif\n",
                       defs_include, header_include, header_include);
}

std::string render_textual_mock_public_header(const TextualMockPublicHeaderRenderSpec &spec) {
    const std::string defs_include     = spec.include_definitions_source
                                             ? include_literal_relative_to(spec.public_header_output_path, spec.definitions_source_path)
                                             : std::string{};
    const std::string registry_include = include_literal_relative_to(spec.public_header_output_path, spec.registry_header_output_path);
    const std::string impl_include = include_literal_relative_to(spec.public_header_output_path, spec.implementation_header_output_path);
    const std::string api_header =
        spec.include_definitions_source && spec.mock_api_header.empty() ? std::string{"gentest/mock.h"} : spec.mock_api_header;
    std::string header;
    header.reserve(defs_include.size() + registry_include.size() + impl_include.size() + api_header.size() + 240);
    header.append("// This file is auto-generated by gentest_codegen.\n"
                  "// Do not edit manually.\n"
                  "#pragma once\n\n");
    if (spec.include_definitions_source && spec.suppress_gentest_auto_mock_include) {
        header.append("#define GENTEST_NO_AUTO_MOCK_INCLUDE 1\n");
    }
    if (spec.include_definitions_source && spec.suppress_gentest_expect_call_macros) {
        header.append("#define GENTEST_NO_EXPECT_CALL_MACROS 1\n");
    }
    if (spec.include_definitions_source) {
        fmt::format_to(std::back_inserter(header), "#include \"{}\"\n#include \"{}\"\n", api_header, defs_include);
    }
    if (spec.include_definitions_source && spec.suppress_gentest_expect_call_macros) {
        header.append("#undef GENTEST_NO_EXPECT_CALL_MACROS\n");
    }
    if (spec.include_definitions_source && spec.suppress_gentest_auto_mock_include) {
        header.append("#undef GENTEST_NO_AUTO_MOCK_INCLUDE\n");
    }
    fmt::format_to(std::back_inserter(header), "\n#include \"{}\"\n#include \"{}\"\n", registry_include, impl_include);
    return header;
}
...
std::optional<std::string> render_module_wrapper_source(const fs::path &source_path, const std::vector<const MockClassInfo *> &source_mocks,
    ...
    if (global_include_location.has_value()) {
        ...
        if (needs_registration_header) {
            rendered.append(render_module_registration_support_include_block(needs_full_registration_support));
            rendered.push_back('\n');
        }
        if (needs_mock_api_include) {
...
struct RenderedSameModuleRegistrationSource {
    std::string                           source;
...
RenderedSameModuleRegistrationSource render_same_module_registration_source(std::string_view module_name,
    ...
    rendered.reserve(module_name.size() + registration_header_name.size() + (source_mocks.size() * 512) + 640);
    rendered.append("// This file is auto-generated by gentest_codegen.\n");
    rendered.append("// Do not edit manually.\n\n");
    rendered.append("module;\n");
    if (!source_mocks.empty()) {
        if (measure_mock_render) {
            result.api_include_started = std::chrono::steady_clock::now();
        }
        rendered.append(render_module_mock_api_include_block());
        if (measure_mock_render) {
            result.api_include_finished = std::chrono::steady_clock::now();
            result.recorded_api_include = true;
        }
    }
    rendered.append(render_module_registration_support_include_block(needs_full_registration_support));
    rendered.append("\nmodule ");
    ...
    if (!source_mocks.empty()) {
        if (measure_mock_render) {
            result.attachment_started = std::chrono::steady_clock::now();
        }
        rendered.append(render_module_mock_api_import_block());
        rendered.push_back('\n');
        std::vector<const MockClassInfo *> sorted_mocks = source_mocks;
        std::ranges::sort(sorted_mocks, {}, [](const MockClassInfo *mock) { return mock->qualified_name; });
        for (const auto *mock : sorted_mocks) {
            rendered.append(render::render_module_mock_attachment(*mock));
            rendered.push_back('\n');
        }
        if (measure_mock_render) {
            result.attachment_finished  = std::chrono::steady_clock::now();
            result.recorded_attachments = true;
        }
    }
    if (has_registrations) {
        rendered.append("#define GENTEST_TU_REGISTRATION_HEADER_NO_PREAMBLE 1\n");
        rendered.append("#include \"");
        rendered.append(registration_header_name);
        rendered.append("\"\n");
        rendered.append("#undef GENTEST_TU_REGISTRATION_HEADER_NO_PREAMBLE\n");
    } else {
        rendered.append("// No gentest registrations were discovered for this module unit.\n");
    }
    return result;
}
...
bool write_file_atomic_if_changed(const fs::path &path, std::string_view content) {
    ...
    if (!try_write_tmp(tmp_path)) {
        ...
        if (!try_write_tmp(tmp_path)) {
            log_err("gentest_codegen: failed to create a temporary output file for '{}'\n", path.string());
            return false;
        }
    }
    ...
    std::error_code ec;
#if defined(_WIN32)
    const bool destination_exists = fs::exists(path, ec);
    if (ec) {
...
bool write_module_registration_manifest(const CollectorOptions &opts) {
    ...
    for (std::size_t idx = 0; idx < opts.sources.size(); ++idx) {
        ...
        fmt::format_to(std::back_inserter(manifest),
                       ...
                       "      \"depfile\": \"{}\"\n"
                       "    }}{}\n",
                       render::escape_string(registration_output.generic_string()), render::escape_string(module_name),
                       render::escape_string(source_path.generic_string()), render::escape_string(context_id),
...
bool write_textual_wrapper_manifest(const CollectorOptions &opts) {
    ...
    fmt::format_to(std::back_inserter(manifest), "{{\n  \"schema\": \"{}\",\n  \"sources\": [\n", artifact_manifest::kSchema);
    for (std::size_t idx = 0; idx < opts.sources.size(); ++idx) {
        const fs::path    wrapper_path  = textual_wrapper_manifest_path(opts, idx);
...
bool validate_generated_artifact_outputs(const CollectorOptions &opts, std::string &error) {
    ...
    if (opts.source_root.has_value() && !opts.source_root->empty()) {
        add_input(*opts.source_root, "source root input", fmt::format("option '--source-root' ('{}')", opts.source_root->string()));
    }
    ...
    if (!opts.mock_registration_manifest_path.empty()) {
        add_input(opts.mock_registration_manifest_path, "mock registration manifest input",
                  fmt::format("option '--mock-registration-manifest' ('{}')", opts.mock_registration_manifest_path.string()));
    }
    if (opts.compilation_database.has_value()) {
        const fs::path compilation_database_file = *opts.compilation_database / "compile_commands.json";
        add_input(compilation_database_file, "compilation database input",
                  fmt::format("option '--compdb' ('{}')", compilation_database_file.string()));
    }
    if (opts.clang_scan_deps_executable.has_value() && !opts.clang_scan_deps_executable->empty()) {
        add_input(*opts.clang_scan_deps_executable, "clang-scan-deps executable",
                  fmt::format("option '--clang-scan-deps' ('{}')", opts.clang_scan_deps_executable->string()));
    }
    ...
    for (const auto &artifact : artifacts) {
        const auto input = std::ranges::lower_bound(inputs, artifact.canonical_path, {}, &ReservedInput::canonical_path);
...
bool validate_timing_json_dependency_collision(const CollectorOptions &opts, const std::vector<std::string> &dependencies,
                                               const std::vector<TimingJsonProtectedPath> &protected_paths, std::string &error) {
    error.clear();
    if (!opts.timing_json_path.has_value() || opts.timing_json_path->empty()) {
        return true;
    }

    const std::string timing_key = casefolded_path_key(*opts.timing_json_path);
    for (const auto &dependency : dependencies) {
        if (dependency.empty()) {
            continue;
        }
        if (casefolded_path_key(fs::path{dependency}) != timing_key) {
            continue;
        }
        error = fmt::format("timing JSON output at canonical path '{}' conflicts with discovered dependency '{}'; "
                            "--timing-json must not replace a parsed dependency",
                            timing_key, dependency);
        return false;
    }
    for (const auto &protected_path : protected_paths) {
        if (protected_path.path.empty() || casefolded_path_key(protected_path.path) != timing_key) {
            continue;
        }
        error = fmt::format("timing JSON output at canonical path '{}' conflicts with {} '{}'; "
                            "--timing-json must not replace a derived codegen path",
                            timing_key, protected_path.description, protected_path.path.string());
        return false;
    }
    return true;
}
...
int emit_registration_outputs(const CollectorOptions &opts, const std::vector<TestCaseInfo> &cases,
                              const std::vector<FixtureDeclInfo> &fixtures, const std::vector<MockClassInfo> &mocks,
                              std::vector<ModuleMockRenderTiming> *module_mock_render_timings) {
    ...
    if (module_mock_render_timings != nullptr) {
        module_mock_render_timings->assign(opts.sources.size(), ModuleMockRenderTiming{});
    }
    ...
    if (!opts.tu_output_dir.empty()) {
        ...
        std::unordered_map<std::string, std::vector<const MockClassInfo *>> registration_mocks_by_module;
        ...
        for (const auto &mock : mocks) {
            if (mock.definition_kind == MockClassInfo::DefinitionKind::NamedModule) {
                if (!opts.module_registration_outputs.empty()) {
                    if (mock.definition_module_name.empty()) {
                        log_err("gentest_codegen: named-module mock '{}' has no definition module name\n", mock.qualified_name);
                        return 1;
                    }
                    registration_mocks_by_module[mock.definition_module_name].push_back(&mock);
                } else if (!mock.definition_file.empty()) {
        ...
        if (!registration_mocks_by_module.empty()) {
            std::unordered_map<std::string, std::size_t> module_source_indices;
            module_source_indices.reserve(opts.module_interface_names_by_source.size());
            for (std::size_t idx = 0; idx < opts.sources.size(); ++idx) {
                const auto module_it = opts.module_interface_names_by_source.find(opts.sources[idx]);
                if (module_it == opts.module_interface_names_by_source.end() || module_it->second.empty()) {
                    continue;
                }
                const auto emplaced = module_source_indices.emplace(module_it->second, idx);
                if (!emplaced.second && registration_mocks_by_module.contains(module_it->second)) {
                    log_err("gentest_codegen: multiple module registration inputs provide module '{}' for mock registration manifest\n",
                            module_it->second);
                    return 1;
                }
            }
            for (const auto &[module_name, module_mocks] : registration_mocks_by_module) {
                const auto source_it = module_source_indices.find(module_name);
                if (source_it == module_source_indices.end()) {
                    log_err(
                        "gentest_codegen: mock registration manifest references module '{}' but no module registration input provides it\n",
                        module_name);
                    return 1;
                }
                auto &source_mocks = per_source[normalize_path_key(fs::path(opts.sources[source_it->second]))].direct_module_mocks;
                source_mocks.insert(source_mocks.end(), module_mocks.begin(), module_mocks.end());
            }
        }
        ...
        parallel_for(opts.sources.size(), jobs, [&](std::size_t idx) {
            ...
            if (!opts.textual_wrapper_outputs.empty()) {
                if (is_module_interface_source(opts, source_path)) {
                    log_err("gentest_codegen: --textual-wrapper-output does not support named module source '{}'\n", source_path.string());
                    statuses[idx] = 1;
                    return;
                }
                const fs::path textual_wrapper_out = resolve_textual_wrapper_output(opts, idx);
                if (!ensure_parent_dir(textual_wrapper_out)) {
                    statuses[idx] = 1;
                    return;
                }
                const TextualWrapperRenderSpec textual_wrapper_spec{
                    .included_source_path  = source_path,
                    .wrapper_output_path   = textual_wrapper_out,
                    .generated_header_path = header_out,
                };
                const std::string textual_wrapper_source = opts.discover_mocks
                                                               ? render_textual_mock_defs_wrapper_source(textual_wrapper_spec)
                                                               : render_textual_suite_wrapper_source(textual_wrapper_spec);
                if (!write_file_atomic_if_changed(textual_wrapper_out, textual_wrapper_source)) {
                    statuses[idx] = 1;
                    return;
                }
            }
            ...
            if (is_module_interface_source(opts, source_path)) {
                ...
                if (!opts.module_registration_outputs.empty()) {
                    if (needs_mock_codegen_include) {
                        log_err("gentest_codegen: module registration mode does not support header-like mock auto-injection for '{}'\n",
                                source_path.string());
                        statuses[idx] = 1;
                        return;
                    }
                    const auto module_it = opts.module_interface_names_by_source.find(opts.sources[idx]);
                    if (module_it == opts.module_interface_names_by_source.end() || module_it->second.empty()) {
                        log_err("gentest_codegen: missing module name for module registration input '{}'\n", source_path.string());
                        statuses[idx] = 1;
                        return;
                    }
                    const fs::path module_registration_out = resolve_module_registration_output(opts, idx);
                    if (!ensure_parent_dir(module_registration_out)) {
                        statuses[idx] = 1;
                        return;
                    }
                    const auto rendered_registration = render_same_module_registration_source(
                        module_it->second, header_out.filename().string(), source_mocks, !tu_cases.empty() || !tu_fixtures.empty(),
                        needs_full_registration_support, module_mock_render_timings != nullptr);
                    if (module_mock_render_timings != nullptr &&
                        (rendered_registration.recorded_api_include || rendered_registration.recorded_attachments)) {
                        (*module_mock_render_timings)[idx] = ModuleMockRenderTiming{
                            .api_include_started  = rendered_registration.api_include_started,
                            .api_include_finished = rendered_registration.api_include_finished,
                            .attachment_started   = rendered_registration.attachment_started,
                            .attachment_finished  = rendered_registration.attachment_finished,
                            .registration_output  = module_registration_out,
                            .module_name          = module_it->second,
                            .recorded_api_include = rendered_registration.recorded_api_include,
                            .recorded_attachments = rendered_registration.recorded_attachments,
                        };
                    }
                    if (!write_file_atomic_if_changed(module_registration_out, rendered_registration.source)) {
                        statuses[idx] = 1;
                    }
                } else {
                    const auto wrapper_source = render_module_wrapper_source(source_path, source_mocks, needs_mock_codegen_include,
                                                                             registration_header_name, needs_full_registration_support);
...
int emit_mock_outputs(const CollectorOptions &opts, const std::vector<MockClassInfo> &mocks) {
    std::string artifact_output_error;
    if (!validate_generated_artifact_outputs(opts, artifact_output_error)) {
        log_err("gentest_codegen: {}\n", artifact_output_error);
        return 1;
    ...
    const bool mocks_consumed_by_registration = !opts.mock_registration_manifest_path.empty();
    if ((!mocks_consumed_by_registration && !mocks.empty()) || have_mock_paths) {
        if (!have_mock_paths) {
        ...
        if (!opts.mock_public_header_path.empty()) {
            if (!ensure_parent_dir(opts.mock_public_header_path)) {
                return 1;
            }
            const TextualMockPublicHeaderRenderSpec public_header_spec{
                .definitions_source_path           = fs::path{opts.sources.front()},
                .public_header_output_path         = opts.mock_public_header_path,
                .registry_header_output_path       = opts.mock_registry_path,
                .implementation_header_output_path = opts.mock_impl_path,
                .mock_api_header            = opts.mock_backend == MockBackend::Gentest ? std::string{"gentest/mock.h"} : std::string{},
                .include_definitions_source = opts.mock_backend == MockBackend::Gentest,
                .suppress_gentest_auto_mock_include  = true,
                .suppress_gentest_expect_call_macros = false,
            };
            const std::string public_header = render_textual_mock_public_header(public_header_spec);
            if (!write_file_atomic_if_changed(opts.mock_public_header_path, public_header)) {
                return 1;
            }
        }
...
int emit(const CollectorOptions &opts, const std::vector<TestCaseInfo> &cases, const std::vector<FixtureDeclInfo> &fixtures,
    ...
    const int registration_status = emit_registration_outputs(opts, cases, fixtures, mocks);
    if (registration_status != 0) {
        return registration_status;
    }
    return emit_mock_outputs(opts, mocks);
}
